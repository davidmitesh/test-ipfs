...
contract licenseValidation{
   ...
   function addContact(string memory _hashvalue) public returns (uint id_){
        id_to_contact[id] = Contact(false,false,_hashvalue);
        id++;
        return id-1;
    }
   ...
   function getContact(uint _id) view public returns (bool _trialpass,bool _writtenpass) {
       require(_id < id);
        return (id_to_contact[_id].trialpass,id_to_contact[_id].writtenpass);
    }
